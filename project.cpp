...
void updateScene()
{
    ...
    if (-INC_SPEED < speed && speed < INC_SPEED)
        return; //"speed<INC_SPEED" because if again accelerated, the bike stops moving if we put speed>INC_SPEED since speed value crosses INC_SPEED
    if (speed < 0.0f)
...
GLfloat angleSum(GLfloat a, GLfloat b)
{
    a += b;
    if (a < 0)
        return a + 2 * PI; // If returned angle is towards left and lesser than its threshold value, increment its value to zero & do nothing
    else if (a > 2 * PI)
        return a - 2 * PI; // If returned angle is towards right and greater than its threshold value, decrement its value to zero & do nothing
    else
        return a; // If nothing, return the radians equivalent of angle 'a'
}
/************************************************
...
void welcome_window()
{
    ...
    bitmap_output(-1.25, 1.8, 0.50, "********SINHGAD INSTITUTES********");
    bitmap_output(-0.6, 1.6, 0.50, "********NARHE PUNE********");
